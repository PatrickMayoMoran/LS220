...
/*
  RECURSIVE BASE CASE:
    When there is only one row, there is only one way
*/

function chaosInTheGrid(grid) {
  function pathsToSquare(row, col) {
    if (row === 0 || col === 0) {
      return 1;
    }

    return pathsToSquare(row - 1, col) + pathsToSquare(row, col - 1);
  }

  return pathsToSquare(grid.length - 1, grid[0].length - 1);
}
